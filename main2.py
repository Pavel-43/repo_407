...
class ProductsWindow(QMainWindow):
    ...
    def search(self):
        ...
        self.load_products(query, (f"%{search_text}%",) * 5)
    ...
    def create_card(self, product):
        ...
        ui.label_price_value.setText(f'<span style="text-decoration: line-through; color: red;">{p:.2f}</span> {p * (100 - d) / 100:.2f}' if d > 0
            else f'{p:.2f}')
    ...
    def open_add(self):
        self.w = ProductForm(self)
        self.w.show()

    def open_edit(self):
        self.w = ProductForm(self, self.selected_id)
        self.w.show()
